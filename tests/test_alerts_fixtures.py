...
class TestETradeAlertsAsyncWithFixtures:
    ...
    @respx.mock
    async def test_list_alerts_xml(self):
    ...
    @respx.mock
    async def test_list_alert_details_xml(self):
    ...
    @respx.mock
    async def test_delete_alert_xml(self):
        """Test async delete_alert with XML fixture."""
        xml_response = load_fixture("DeleteAlertsResponse.xml")
        
        url = "https://api.etrade.com/v1/user/alerts/12345.xml"
        respx.delete(url).mock(return_value=Response(200, text=xml_response))
