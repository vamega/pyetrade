...
class TestETradeOAuthAsync:
    ...
    async def test_get_request_token(self):
        ...
        # Here we are using real AsyncOAuth1Client with respx.
        ...
        respx.route(url=url).mock(return_value=Response(200, text=body))
    ...
    async def test_get_access_token(self):
        ...
        oauth.session = AsyncOAuth1Client("key", "secret", token="req_token", token_secret="req_secret")
...
class TestETradeAccessManagerAsync:
    ...
    @respx.mock
    async def test_renew_access_token(self):
        manager = ETradeAccessManager("key", "secret", "token", "token_secret")
        
        url = "https://api.etrade.com/oauth/renew_access_token"
        respx.get(url).mock(return_value=Response(200, text="Access Token has been renewed"))
    ...
    async def test_revoke_access_token(self):
        ...
        respx.get(url).mock(return_value=Response(200, text="Revoked"))
